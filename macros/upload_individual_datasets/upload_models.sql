        ...
        {% set model_values %}
            ...
            {% for model in models -%}
                ...
                (
                    ...
                    '{{ model_copy.checksum.checksum  | replace('\\', '\\\\') }}'{% if var('dbt_artifacts_environment_aware', false) %}||'|'||'{{ env_var('DBT_CLOUD_ENVIRONMENT_NAME', '') }}'||'|'||'{{ env_var('DBT_CLOUD_ENVIRONMENT_TYPE', '') }}'{% endif %}, {# checksum #} 
